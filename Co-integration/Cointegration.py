...
for nlag in range(1,50):
    ...
    cov_params = np.kron(covariance, sigma_hat)  # covariance matrix of parameters
    ...
    tvalue = cov_params / bse  # t-statistic for a given parameter estimate
    nobs2 = nobs / 2.0
    ...
    BIC = np.log(np.linalg.det(sigma_hat)) +  (K_dash/ nobs) * log(nobs) # log(sigma_hat) + K_dash/T*log(T)
